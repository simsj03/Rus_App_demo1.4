<?php
	require_once 'guest-db.php';
	$guest = new guest();
	session_start();

	//Handle guest login request
	if (isset($_POST['action']) && $_POST['action'] == 'guestLogin') {
	  $username = $guest->test_input($_POST['username']);
	  $password = $guest->test_input($_POST['password']);

	  $hpassword = sha1($password);

	  $loggedInGuest = $guest->guest_login($username,$hpassword);

	  if ($loggedInGuest != null) {
	    echo 'guest_login';
	    $_SESSION['username'] = $username;
	  } else {
	    echo $guestn->showMessage('danger','Username or Password is Incorrect!');
	  }
	}  
	...
	if (isset($_POST['details_id'])) {
	  $id = $_POST['details_id'];
	  $data = $guest->fetchUserDetailsByID($id);
	...
	if (isset($_POST['action']) && $_POST['action'] == 'fetchAllNotes') {
	  ...
	  if ($note) {
	    ...
	    echo '<h3 class="text-center text-secondary">:( No notes written</h3>';
	  }
	}
	...
	if (isset($_POST['notification_id'])) {
	  ...
	  $guest->removeNotification($id);
	}

	//Check notification
	if (isset($_POST['action']) && $_POST['action'] == 'notificationCheck') {
	  if ($guest->fetchNotification()) {
	...
	if (isset($_POST['message'])) {
	  ...
	  $guest->replyFeedback($uid,$message);
	  $guest->feedbackReplied($fid);
	}
